...
#define kBackupCompleteKey                      @"Make a Backup"
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	[backupOptions addObject:kBackupCompleteKey];
...
- (void)exportFileCreatedForServer:(NSNotification *)notification
{
	NSLog(@">>> Entering %s <<<", __PRETTY_FUNCTION__);
	...
	[[DataController sharedDataController] prepareUploadToServer:zipPath];
...
- (void)prepareExportFileToServer
{
	NSLog(@">>> Entering %s <<<", __PRETTY_FUNCTION__);
	
	NSNotificationCenter *notificationCenter = [NSNotificationCenter defaultCenter];
	[notificationCenter removeObserver:self name:kDidCreateExportFileNotification object:nil];
	[notificationCenter addObserver:self selector:@selector(exportFileCreatedForServer:) name:kDidCreateExportFileNotification object:nil];
...
//			}
	...
	else if ([rowKey isEqualToString:kBackupCompleteKey])
	{
        //we need to move the file to backup server for later download on new application
        // do following
        // #1. Show activity spinner for request in progress
        // #2. export the data and upload to server
        // #3. wait for request to be finished and show notification to user on completion
        
        //[self saveExportFile];
        [self prepareExportFileToServer];
